  ...
  def to_bazelrc_lines(
    ...
    if self.backend == Backend.CPU:
      ...
      if dpav.cuda_version:
        rc.append(
            f"build:cuda --repo_env CUDA_VERSION={dpav.cuda_version}"
        )
        # Convert CUDA version string (e.g. "12.8.1") to numeric format (e.g. "12081")
        cuda_ver_parts = dpav.cuda_version.split('.')
        cuda_ver_numeric = f"{cuda_ver_parts[0]}{cuda_ver_parts[1].zfill(2)}" \
                           f"{cuda_ver_parts[2].zfill(1)}"
        rc.append(f"build:cuda --copt=-DCUDA_VERSION={cuda_ver_numeric}")
        rc.append(
