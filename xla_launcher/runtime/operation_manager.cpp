...
void OperationManager::WaitForDevices(absl::Span<const std::string> devices) {
  ...
  for (const std::string& device_str : devices) {
    VLOG(5) << "Blocking new operations on " << device_str;
    auto lock = op_counters_.at(device_str).BlockNewOperations();
    locks.emplace_back(std::move(lock));

    VLOG(3) << "Waiting for device execution for " << device_str
            << " to finish";
    op_counters_.at(device_str).Wait();
    VLOG(3) << "Finished operations on device " << device_str;
  }
}
...
void OperationManager::Counter::Increment() {
  ...
  VLOG(5) << "Incremented operations for " << device_ << " to " << current;
}

void OperationManager::Counter::Decrement() {
  auto current = count_.fetch_sub(1, std::memory_order_acq_rel) - 1;
  VLOG(5) << "Decremented operations for " << device_ << " to " << current;

  if (current == 0) {
    std::unique_lock cv_lock(cv_mu_);
    VLOG(3) << "All operations complete for " << device_;
    cv_.notify_all();
  }
}
...
void OperationManager::Counter::Wait() {
  VLOG(5) << "Waiting for " << count_ << " operations on " << device_;
  std::unique_lock cv_lock(cv_mu_);
  cv_.wait(
    cv_lock, [this] { return count_.load(std::memory_order_acquire) == 0; });
  VLOG(5) << "Done waiting for " << device_;
}
