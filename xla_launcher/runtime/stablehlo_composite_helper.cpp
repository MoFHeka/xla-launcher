...
struct BoundaryMetadata {
 ...
 private:
  template <typename T>
  static bool CopyJsonValue(
    const Json::Value& j, llvm::StringRef key, Json::ValueType expected_type,
    T& to) {
    if (!j.isMember(key.str())) {
      return false;
    }
    const Json::Value& value = j[key.str()];
  ...
  static std::unique_ptr<BoundaryMetadata> Build(const Json::Value& j) {
    BoundaryMetadata metadata;

    bool is_valid_metadata_json =
      CopyJsonValue(j, "name", Json::stringValue, metadata.name)
      && CopyJsonValue(j, "id", Json::stringValue, metadata.id)
      && CopyJsonValue(j, "pos", Json::intValue, metadata.pos)
      && CopyJsonValue(j, "is_input", Json::booleanValue, metadata.is_input);

    if (!is_valid_metadata_json) {
      return nullptr;
    }

    if (j.isMember("attr") && j["attr"].isObject()) {
      const Json::Value& attrs_j = j["attr"];
      for (const auto& key : attrs_j.getMemberNames()) {
        metadata.attrs.insert({key, attrs_j[key]});
      }
    }
 ...
 private:
  ...
  mlir::FailureOr<mlir::Attribute> BuildAttrFromJson(
    ...
    switch (json_value.type()) {
      case Json::intValue:
        return builder.getI64IntegerAttr(json_value.asInt64());
      case Json::realValue:
        return builder.getF32FloatAttr(json_value.asFloat());
      case Json::booleanValue:
        return builder.getBoolAttr(json_value.asBool());
      case Json::stringValue:
        return builder.getStringAttr(json_value.asString());
      case Json::arrayValue: {
        ...
        // 检查数组类型是否一致
        Json::ValueType head_type = json_value[0].type();
        bool is_homogeneous = true;
        for (const auto& el : json_value) {
          if (el.type() != head_type) {
        ...
        switch (head_type) {
          case Json::intValue: {
            ...
            for (const auto& el : json_value) {
              values.push_back(el.asInt64());
            }
            return builder.getI64TensorAttr(values);
          }
          case Json::realValue: {
            llvm::SmallVector<float> values;
            for (const auto& el : json_value) {
              values.push_back(el.asFloat());
            }
            return mlir::DenseFPElementsAttr::get(
              mlir::RankedTensorType::get(
                json_value.size(), builder.getF32Type()),
              values);
          }
          case Json::booleanValue: {
            llvm::SmallVector<bool> values;
            for (const auto& el : json_value) {
              values.push_back(el.asBool());
            }
            return mlir::DenseIntElementsAttr::get(
              mlir::RankedTensorType::get(
                json_value.size(), builder.getI1Type()),
              values);
          }
          default:
            return op->emitError()
                   << "invalid JSON to MLIR: invalid array type. arrays must "
                      "be 1-D homogeneous arrays of supported primitive types";
        }
      }
      default:
        return op->emitError()
  ...
  mlir::FailureOr<std::pair<
    llvm::SmallVector<mlir::Value>, llvm::SmallVector<mlir::Operation*>>>
  GetBoundaryArgsAndOps(
    ...
    llvm::SmallVector<mlir::Operation*> processing(
      boundary_output_ops.begin(), boundary_output_ops.end());
  ...
  mlir::func::FuncOp BuildStableHLOCompositeImplFunc(
    ...
    builder.createBlock(
      &impl_func.getBody(), impl_func.begin(), arg_types, arg_locs);
    ...
    for (mlir::Operation* op : boundary_output_ops) {
      results.append(
        mapping.lookup(op)->getResults().begin(),
  ...
  mlir::FailureOr<mlir::Operation*> BuildStableHLOCompositeOp(
    ...
    mlir::FailureOr<mlir::DictionaryAttr> attributes_or =
      BuildDictionaryAttrFromJsonMap(
        builder, boundary_output_op, metadata.attrs);
    ...
    mlir::Operation* composite_op =
      builder.create<mlir::stablehlo::CompositeOp>(
        boundary_output_op->getLoc(), impl_func.getFunctionType().getResults(),
        args, metadata.name, *attributes_or, impl_func.getSymName());
    return composite_op;
  }
};
