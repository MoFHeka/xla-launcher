...
xla::XlaOp ConvertBinaryOpResult(
  xla::XlaOp op1, xla::XlaOp op2, xla::XlaOp result) {
...
xla::XlaComputation CreateMinMaxComputation(
  const std::string& name, xla::PrimitiveType value_type,
  xla::PrimitiveType index_type, bool is_min) {
  xla::XlaBuilder builder(name);
...
xla::XlaOp XlaHelpers::BroadcastDimensions(
  ...
  return xla::BroadcastInDim(
    input, bcast_sizes, GetAllDimensions(bcast_sizes.size()));
}
...
XlaHelpers::DynamicSize XlaHelpers::GetDimensionsSize(
  ...
  xla::PrimitiveType size_type = GetShapeDimensionType();
  xla::XlaOp size;
  int64_t size_scalar = 1;
  for (auto& input : inputs) {
    const xla::Shape* shape = GetShape(input).value();
...
XlaHelpers::MinMax XlaHelpers::MinMaxValues(xla::PrimitiveType type) {
  switch (type) {
    ...
    case xla::PrimitiveType::U32:
      return {
        static_cast<int64_t>(std::numeric_limits<uint32_t>::lowest()),
    ...
    case xla::PrimitiveType::U64:
      return {
        static_cast<int64_t>(std::numeric_limits<uint64_t>::lowest()),
        static_cast<int64_t>(std::numeric_limits<uint64_t>::max())};
    case xla::PrimitiveType::F16:
      return {
        static_cast<float>(std::numeric_limits<xla::half>::lowest()),
...
xla::XlaComputation XlaHelpers::CreateAddComputation(xla::PrimitiveType type) {
  return CreateComputation(
    "AddComputation", type, [&](xla::XlaOp x, xla::XlaOp y) {
      return type == xla::PrimitiveType::PRED ? xla::Or(x, y) : xla::Add(x, y);
    });
}
...
xla::XlaComputation XlaHelpers::CreateMaxAndArgMaxComputation(
  ...
  return CreateMinMaxComputation(
    "MaxAndArgMaxComputation", value_type, index_type, /*is_min=*/false);
}

std::vector<int64_t> XlaHelpers::SizesOfXlaOp(xla::XlaOp op) {
  const xla::Shape* op_shape = GetShape(op).value();
  return std::vector<int64_t>(
    op_shape->dimensions().begin(), op_shape->dimensions().end());
}

xla::PrimitiveType XlaHelpers::TypeOfXlaOp(xla::XlaOp op) {
  return GetShape(op).value()->element_type();
}

xla::XlaOp XlaHelpers::ReshapeToRank(
  xla::XlaOp input, int64_t expected_rank, int64_t offset) {
  const xla::Shape* shape = GetShape(input).value();
  ...
  dimensions.insert(
    dimensions.end(), shape->dimensions().begin(), shape->dimensions().end());
...
XlaHelpers::GetDynamicReshapeInfo(
  ...
  if (info.output_shape.dimensions_size() > 0) {
    ...
    for (int64_t i = 0; i < output_sizes.size(); ++i) {
      XLA_CHECK_LE(
        out_size,
        size_prod_until_dyndim / input_shape.dimensions(input_dyndim_idx))
...
xla::XlaOp XlaHelpers::DynamicReshape(
  ...
  const xla::Shape* input_shape = GetShape(input).value();
  ...
  auto info = GetDynamicReshapeInfo(*input_shape, output_sizes);
  if (info) {
    return xla::ReshapeWithInferredDimension(
      input, output_sizes, info->dynamic_dimension);
  }
  return xla::Reshape(input, output_sizes);
}

xla::XlaOp XlaHelpers::DynamicReshapeAs(
  xla::XlaOp input, const xla::Shape& shape) {
  const xla::Shape* input_shape = GetShape(input).value();
  int64_t dynamic_dimension = GetDynamicDimension(shape);
  if (dynamic_dimension >= 0) {
    return xla::ReshapeWithInferredDimension(
      input, shape.dimensions(), dynamic_dimension);
  }
...
xla::XlaOp XlaHelpers::DynamicUnboundedReshape(
  ...
  bool output_sizes_unbounded_dynamic = std::any_of(
    ...
    [](int64_t size) { return size == xla::Shape::kUnboundedSize; });
  XLA_CHECK(
    input_shape->is_unbounded_dynamic() || output_sizes_unbounded_dynamic)
  ...
  const xla::Shape* aux_input_shape = GetShape(aux_input).value();
  ...
  for (int i = 0; i < output_sizes.size(); i++) {
    if (output_sizes[i] == xla::Shape::kUnboundedSize) {
      ...
      get_dim_ops.push_back(
        XlaHelpers::ScalarValue<int32_t>(output_sizes[i], aux_input.builder()));
    }
  }
  ...
  return xla::CustomCall(
    ...
    xla::ShapeUtil::MakeShape(
      aux_input_shape->element_type(), output_sizes, output_dynamic));
}

bool XlaHelpers::SameStaticDimensions(
...
xla::XlaOp XlaHelpers::Flatten(xla::XlaOp input, xla::Shape* input_shape) {
  MaybePtr<xla::Shape> input_shape_tmp(input_shape);
  input_shape_tmp = const_cast<xla::Shape*>(GetShape(input).value());
...
xla::XlaOp XlaHelpers::FlattenDimRange(
  xla::XlaOp input, int64_t start, int64_t range, xla::Shape* input_shape) {
  MaybePtr<xla::Shape> input_shape_tmp(input_shape);
  input_shape_tmp = const_cast<xla::Shape*>(GetShape(input).value());
...
xla::XlaOp XlaHelpers::LinearInterpolation(
  xla::XlaOp value0, xla::XlaOp value1, double alpha) {
  const xla::Shape* shape = GetShape(value0).value();
...
xla::PrimitiveType XlaHelpers::PromoteType(
  xla::PrimitiveType type1, xla::PrimitiveType type2) {
  if (type1 == type2) {
    return type1;
  }
  ...
  if (
    xla::primitive_util::IsIntegralType(type1)
...
void ExtractDimensionSizesAndDynamicDimensionsFromShape(
  ...
  dimensions.insert(
    dimensions.end(), shape.dimensions().begin(),
    shape.dimensions().begin() + count);
  dynamic_dimensions.insert(
    dynamic_dimensions.end(), shape.dynamic_dimensions().begin(),
    shape.dynamic_dimensions().begin() + count);
}

}  // namespace

xla::Shape XlaHelpers::GetPromotedShape(
  const xla::Shape& shape1, const xla::Shape& shape2) {
  std::vector<int64_t> dimensions;
  std::vector<bool> dynamic_dimensions;
  ...
  for (size_t i = 0; i < min_size; i++) {
    ...
    int64_t dynamic_dim1 =
      shape1.dynamic_dimensions()
        [shape1.dynamic_dimensions().size() - min_size + i];
    int64_t dim2 =
      shape2.dimensions()[shape2.dimensions().size() - min_size + i];
    int64_t dynamic_dim2 =
      shape2.dynamic_dimensions()
        [shape2.dynamic_dimensions().size() - min_size + i];
    ...
    // TODO(torch): Consider replacing the broadcasting logic below with
  ...
  return xla::ShapeUtil::MakeShape(
    shape1.element_type(), dimensions, dynamic_dimensions);
}

std::vector<int64_t> XlaHelpers::getBroadcastDimensions(
  xla::XlaOp op1, xla::XlaOp op2) {
  const xla::Shape* shape1 = GetShape(op1).value();
  const xla::Shape* shape2 = GetShape(op2).value();
  if (
    shape1->dimensions_size() == 0 || shape2->dimensions_size() == 0
...
xla::Shape XlaHelpers::GetPromotedBinaryOpShape(
  const xla::Shape& shape1, const xla::Shape& shape2) {
  ...
  if (XlaHelpers::IsUnboundedDynamismEnabled()) {
    XLA_CHECK(
      !XlaHelpers::IsUnboundedDynamic(shape1)
...
xla::Shape XlaHelpers::GetPromotedDynamicShape(
  const xla::Shape& shape1, const xla::Shape& shape2) {
  std::vector<int64_t> upper_bounds1 = ConvertXlaShapeToVector(shape1);
  std::vector<int64_t> upper_bounds2 = ConvertXlaShapeToVector(shape2);
...
std::pair<xla::XlaOp, xla::XlaOp> XlaHelpers::Promote(
  xla::XlaOp op1, xla::XlaOp op2) {
...
xla::XlaOp XlaHelpers::ImplicitBroadcast(
  xla::XlaOp op, const xla::Shape& op_shape, const xla::Shape& shape) {
  ...
  xla::XlaOp new_op = op;
  if (!std::equal(
        op_shape_dims.begin(), op_shape_dims.end(),
    ...
    //   op_shape =       [3, 4, 5]
    std::vector<int64_t> common_shape_dims(
      shape_dims.begin() + size_delta, shape_dims.end());
  ...
  if (size_delta > 0) {
    ...
    std::vector<int64_t> broadcast_sizes(
      shape_dims.begin(), shape_dims.begin() + size_delta);
...
std::vector<xla::XlaOp> ExtractDimensionSizesWithPadding(
  const xla::XlaOp op, size_t num_dims, int pad_count) {
...
xla::XlaOp XlaHelpers::DynamicBroadcastInDim(
  ...
  const xla::Shape* shape = GetShape(op).value();

  std::vector<int64_t> op_broadcast_dims(shape->dimensions().size());
  std::iota(
    op_broadcast_dims.begin(), op_broadcast_dims.end(),
  ...
  for (auto dim : aux_input_dimensions) {
    if (aux_input_shape->dimensions(dim) != xla::Shape::kUnboundedSize) {
      get_dim_ops.push_back(xla::Reshape(
        XlaHelpers::ScalarValue<int32_t>(
...
XlaHelpers::ImplicitBroadcastWithUnboundedDynamicShapes(
  ...
  auto final_broadcast_dimensions = xla::Max(
    xla::ConcatInDim(op1.builder(), op1_dims, 0),
...
std::vector<int64_t> XlaHelpers::ConvertXlaShapeToVector(
  const xla::Shape& shape) {
  return std::vector<int64_t>(
    shape.dimensions().begin(), shape.dimensions().end());
}

std::vector<std::optional<int64_t>> XlaHelpers::ConvertXlaShapeToOptionalVector(
  const xla::Shape& shape) {
  std::vector<std::optional<int64_t>> dimensions;
  dimensions.reserve(shape.dimensions_size());
  for (auto it = shape.dimensions().begin(); it != shape.dimensions().end();
       ++it) {
    if (*it) {
