...
#ifndef XLA_LAUNCHER_XLA_BUILD_HELPERS_HPP_
#define XLA_LAUNCHER_XLA_BUILD_HELPERS_HPP_
...
class MaybePtr {
 public:
  explicit MaybePtr(T* ptr) : ptr_(ptr) {
    if (ptr_ == nullptr) {
      storage_ = T();
      ptr_ = &storage_.value();
    }
...
class XlaHelpers {
 public:
  ...
  template <class T>
  static xla::XlaOp ScalarValue(
    T scalar_value, xla::PrimitiveType type, xla::XlaBuilder* builder) {
    return xla::ConstantLiteral(builder, ScalarLiteral(scalar_value, type));
  }

  // TODO(He Jia): Implement DLTensor support.
  // static xla::XlaOp ScalarValue(
  //   const DLTensor& scalar_value,
  ...
  static xla::XlaOp LinearInterpolation(
  ...
  static xla::XlaOp CreateReturnValue(
  ...
  static xla::XlaOp ScalarBroadcast(
    ...
    absl::Span<const int64_t> dimensions, xla::XlaBuilder* builder) {
  ...
  template <class T>
  static xla::XlaOp ScalarBroadcast(
    T scalar_value, const xla::Shape& shape, xla::XlaBuilder* builder) {
    return ScalarBroadcast<T>(
      scalar_value, shape.element_type(), shape.dimensions(), builder);
  }
  ...
  static bool IsUnboundedDynamismEnabled() {
    return runtime::sys_util::GetEnvBool(
      "EXPERIMENTAL_XLA_UNBOUNDED_DYNAMISM", false);
  }
  ...
  static bool SameStaticDimensions(
  ...
  static std::vector<int64_t> I64List(const S& input) {
    return std::vector<int64_t>(input.begin(), input.end());
  }
  ...
  static xla::XlaOp ReshapeToRank(
    xla::XlaOp input, int64_t expected_rank, int64_t offset = 0);

  static xla::XlaOp Flatten(
    xla::XlaOp input, xla::Shape* input_shape = nullptr);

  static xla::XlaOp FlattenDimRange(
    xla::XlaOp input, int64_t start, int64_t range,
  ...
  static std::vector<typename Container::value_type> Permute(
    ...
    XLA_CHECK(
      input.size() == permutation.size() && xla::IsPermutation(permutation))
  ...
  static xla::PrimitiveType PromoteType(
  ...
  static std::pair<xla::XlaOp, xla::XlaOp> PromoteValues(
  ...
  // returned.
  static xla::XlaOp ImplicitBroadcast(
  ...
  // TODO(He Jia): We need to emit runtime shape assertions to validate the
  // broadcasting rules are met.
  static std::pair<xla::XlaOp, xla::XlaOp>
  ImplicitBroadcastWithUnboundedDynamicShapes(
  ...
  static std::vector<int64_t> getBroadcastDimensions(
    xla::XlaOp op1, xla::XlaOp op2);
  ...
  static xla::Literal Range(A start, A end, A step) {
    ...
    std::vector<A> accumulated;
    accumulated.reserve(static_cast<size_t>((end - start) / step));
    for (A val = start; (step > 0) ? (val < end) : (val > end); val += step)
      accumulated.push_back(val);
  ...
  static std::vector<int64_t> ConvertXlaShapeToVector(const xla::Shape& shape);

  static std::vector<std::optional<int64_t>> ConvertXlaShapeToOptionalVector(
...
#endif  // XLA_LAUNCHER_XLA_BUILD_HELPERS_HPP_
